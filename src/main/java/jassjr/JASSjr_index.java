...
class JASSjr_index
			...
			for (String line : (Iterable<String>) stream::iterator)
				...
				for (token = lexGetFirst(line); token != null; token = lexGetNext())
					...
					*/
					...
					//porter stemmer
					token = stemmWordsWithRules(token);
	...
	*/
	public static void main(String args[]) 
		...
		public static boolean checkStopWord(String token) throws IOException{

			RemoveStopWords removeStopWords = new RemoveStopWords();
			boolean isStopWord = removeStopWords.checkStopWord(token);
			return isStopWord;
